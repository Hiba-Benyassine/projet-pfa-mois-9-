...
export class LeaveDemandeListComponent implements OnInit {
  demandes: Demande[] = [];
  isAdmin = false;
  currentUserEmail = '';
  successMessage = '';
...
valiRequest(demande: Demande): void {
  // Ask for confirmation first
  if (!confirm('Voulez-vous vraiment valider cette demande ?')) return;

  // Only push id after confirmation
  if (!this.validIds.includes(demande.id)) {
    this.validIds.push(demande.id);
  }
  ...
  // Refresh the list
  this.demandeService.getDemandes(this.currentUserEmail, this.isAdmin)
    .subscribe((list: Demande[]) => {
      this.demandes = list;
    });
}
